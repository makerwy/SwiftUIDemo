...
struct RefreshExampleView: View {
    @ObservedObject private var model = MyTestModel()

    var body: some View {
        NavigationView {
            LBRefreshableScrollView(refreshing: self.$model.loading) {
                VStack(spacing: 0) {
...
struct RefreshExampleView_Previews: PreviewProvider {
